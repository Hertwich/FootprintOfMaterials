...
% Code created 2018-2019
% Edgar Hertwich, edgar.hertwich@ntnu.no
% For Nature Geoscience manuscript https://osf.io/preprints/socarxiv/n9ecw/

% Please cite final paper when acknowledging the use of this code.

% No support provided, use at own risk.

% I am afraid I cannot share the data used for the calculations given the
% use of proprietary of third-party data sources.
...
xiopath='..\..\..\..\Dropbox (Yale_FES)\EX_v3_6_mat\';
%%
year=1995:2016;
% year=2011;
% a=1;
...
%%
s=[13,size(sel_c,1)+size(sel_sec,2),meta.NSECTORS,size(sel_sec,2)+1,size(year,2)];
ind=zeros(s);
...
name.indi_dim3=meta.labsZ(1:n_sec,2);
...
%     IO=endogenize_capital(IO,meta);
    ...
    xsum(:,a)=sec_sum(IO.x,n_sec,1);
    %%
       for i=1:size(sel_sec,2)                         
        ind(:,:,:,i,a)=hypothetical_extraction9(sel_sec(i),IO,n_sec,n_countries,S,GFCF);
        disp(['Extracted sector ',char(meta.labsZ(sel_sec(i),2)),'  ',num2str(year(a))]);
       end
    %%
    
    ind(:,:,:,i+1,a)=hypothetical_extraction9(sel_sec,IO,n_sec,n_countries,S,GFCF);
    %%
    gen(:,:,:,a)=footprint_by_sector(IO,S,sel_c,n_sec);
...
%%
save('TimeSeries190430p.mat', 'ind','indi_flat','indi_sec','gen','heading','names_C','sel_sec','xsum','adj','year','name');
%%
filename=['Timeseries',date,'.xlsx'];
% s(5)=1;
...
%%
xls=[num2cell(1:200)',name.indi_dim3,num2cell(indi_flat)];
xls=[cell(4,2),heading;xls];
xlswrite(filename,xls,'Detail');
clear xls
%%
xls=[heading;num2cell([indi_compact;indi_sum;indi_sec])];
xls=[[cell(4,1);{'Energy','Transport','Materials','Industry','Services','Buildings','AFOLU+','Sum','Extracted Sector','Sector','GCFC','NCFC'}'],xls];
xlswrite(filename,xls,'Summary2');

%%
heading2=cell(4,size(sel_c,1)*s(1)*s(5));
indiM=zeros(meta.NSECTORS,size(sel_c,1)*s(1)*s(5));
for i=1:s(4)
    for j=1:size(sel_c,1)
        for k=1:s(1)
                r=(j-1)*s(1)*s(5)+(k-1)*s(1)+(1:s(5));
                indiM(:,r)=squeeze(ind(k,j,:,i,1:s(5)));
                heading2(1,r)=name.indi_dim2(j);
                heading2(2,r)=name.indi_dim1(k);
                heading2(3,r)=name.indi_dim4(i);
                heading2(4,r)=num2cell(year(1:s(5)));
        end
    end
